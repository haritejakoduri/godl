// Package urlname derives a sensible destination filename for a direct
// HTTP(S) link.
//
// It's more work than it sounds: many links are an opaque ID or token
// (a CDN serving /dld/<uuid>?token=...), so the URL path alone often
// yields a name with no extension. This package falls back to asking
// the server — Content-Disposition, then Content-Type — and finally to
// sniffing the first bytes of the body, rather than saving a file with
// no extension at all.
//
// Separate from the cobra command because the TUI's new-download wizard
// needs the same answer the "godl url" command does.
package urlname
...
// called" exchange. It's a nicety, not the download itself, so it gives
// up quickly and lets the caller fall back to the URL-derived name.
